...
export class World {
  ...
  focus(x, z, viewW, anim) { const from = { x: this.target.x, z: this.target.z, w: this.viewW }; const to = { x, z, w: viewW || this.viewW }; if (!anim || this.reduced) { this.target.set(x, 0, z); this.viewW = to.w; this._cam(); return; } const t0 = performance.now(); const myId = this._focusId = t0; const step = () => { if (this._focusId !== myId) return; const k = Math.min(1, (performance.now() - t0) / 450); const e = 1 - Math.pow(1 - k, 3); this.target.set(from.x + (to.x - from.x) * e, 0, from.z + (to.z - from.z) * e); this.viewW = from.w + (to.w - from.w) * e; this._cam(); if (k < 1) requestAnimationFrame(step); }; step(); }
  ...
  _gestures() {
    const el = this.renderer.domElement; const ptrs = new Map(); let last = null, pd = 0, pa = 0, twist = 0, rotOn = false; this.dragged = false;
    el.addEventListener('pointerdown', e => { try { el.setPointerCapture(e.pointerId); } catch (_) {} const r0 = el.getBoundingClientRect(); const hs = this.heroDraggable && this.hero && !this.heroPath ? this.heroScreen() : null;
      if (hs && ptrs.size === 0 && Math.hypot(hs.x - (e.clientX - r0.left), hs.y - (e.clientY - r0.top)) < 34) { // 캐릭터 위 길게 누르면(0.3초) 끌기 시작
        const pid = e.pointerId, sx = e.clientX, sy = e.clientY; clearTimeout(this._holdT);
        this._holdT = setTimeout(() => { const cur = ptrs.get(pid); if (ptrs.size === 1 && cur && Math.hypot(cur.x - sx, cur.y - sy) < 8) { ptrs.delete(pid); last = null; this.dragHero = { id: pid, orig: this.hero.position.clone() }; if (this.onHeroHold) this.onHeroHold(); } }, 300); } ptrs.set(e.pointerId, { x: e.clientX, y: e.clientY }); if (ptrs.size === 1) { last = { x: e.clientX, y: e.clientY }; this.dragged = false; } if (ptrs.size === 2) { const [a, b] = [...ptrs.values()]; pd = Math.hypot(a.x - b.x, a.y - b.y); pa = Math.atan2(a.y - b.y, a.x - b.x); twist = 0; rotOn = false; last = null; } this._focusId = null; });
    el.addEventListener('pointermove', e => { if (this.dragHero && this.dragHero.id === e.pointerId) { const r0 = el.getBoundingClientRect(); const g = this.groundAt(e.clientX - r0.left, e.clientY - r0.top); if (g) { this.hero.position.set(g.x, 0.05, g.z); } return; } if (!ptrs.has(e.pointerId)) return; ptrs.set(e.pointerId, { x: e.clientX, y: e.clientY }); if (ptrs.size === 1) { if (!last) { last = { x: e.clientX, y: e.clientY }; return; } const dx = e.clientX - last.x, dy = e.clientY - last.y; if (Math.abs(dx) + Math.abs(dy) > 3) this.dragged = true; this.panPx(dx, dy); last = { x: e.clientX, y: e.clientY }; } else if (ptrs.size === 2) { const [a, b] = [...ptrs.values()]; const d = Math.hypot(a.x - b.x, a.y - b.y), an = Math.atan2(a.y - b.y, a.x - b.x); const r = el.getBoundingClientRect(); const cx = (a.x + b.x) / 2 - r.left, cy = (a.y + b.y) / 2 - r.top;
        const before = this.groundAt(cx, cy);
        if (pd) this.viewW = THREE.MathUtils.clamp(this.viewW / (d / pd), 110, 1800);
        let da = an - pa; while (da > Math.PI) da -= 2 * Math.PI; while (da < -Math.PI) da += 2 * Math.PI; twist += da;
        if (!rotOn && Math.abs(twist) > 0.14) { rotOn = true; }   // 약 8도 이상 비틀어야 회전 시작
        if (rotOn) this.az += da;
        this._cam(); if (before) { const after = this.groundAt(cx, cy); if (after) { this.target.x += before.x - after.x; this.target.z += before.z - after.z; this._cam(); } }
        pd = d; pa = an; } });
    const up = e => { clearTimeout(this._holdT); if (this.dragHero && this.dragHero.id === e.pointerId) { const d = this.dragHero; this.dragHero = null; if (this.onHeroDrop) this.onHeroDrop(this.hero.position.clone(), d.orig); return; } ptrs.delete(e.pointerId); if (ptrs.size < 2) { pd = 0; rotOn = false; twist = 0; } if (ptrs.size === 1) { const [p] = ptrs.values(); last = { x: p.x, y: p.y }; } if (ptrs.size === 0) last = null; };
    el.addEventListener('pointerup', up); el.addEventListener('pointercancel', up);
    el.addEventListener('touchmove', e => { if (e.touches.length > 1) e.preventDefault(); }, { passive: false }); document.addEventListener('gesturestart', e => e.preventDefault(), { passive: false }); document.addEventListener('gesturechange', e => e.preventDefault(), { passive: false });
  ...
  setHero(x, z, opts) { if (!this.hero) { this.hero = this.makeFigure({ torso: 0x2f7f86, legs: 0xc9bda4, hair: 0x4a2f1f, pack: 0x7b8a4a, scale: 2.9 }); this.hero.traverse(o => { if (o.isMesh) { o.material = o.material.clone(); o.material.emissive = new THREE.Color(o.material.color).multiplyScalar(0.35); o.material.transparent = true; o.material.depthWrite = true; o.renderOrder = 30; } }); const glow = new THREE.Sprite(new THREE.SpriteMaterial({ map: this._glowTex(), transparent: true, depthWrite: false, depthTest: false, blending: THREE.AdditiveBlending, opacity: 0.55 })); glow.scale.set(6.5, 6.5, 1); glow.position.y = 0.6; glow.renderOrder = 11; this.hero.add(glow); this.hero.userData.glow = glow; const halo = new THREE.Sprite(new THREE.SpriteMaterial({ map: this._glowTex(), transparent: true, depthWrite: false, depthTest: false, blending: THREE.AdditiveBlending, opacity: 0.3 })); halo.scale.set(2.6, 2.6, 1); halo.position.y = 2.2; halo.renderOrder = 11; this.hero.add(halo); const ring = new THREE.Mesh(new THREE.RingGeometry(1.7, 2.5, 40), new THREE.MeshBasicMaterial({ color: 0x6fe3d8, transparent: true, opacity: 0.95, depthWrite: false, depthTest: false, side: THREE.DoubleSide })); ring.renderOrder = 12; ring.rotation.x = -Math.PI / 2; ring.position.y = 0.05; this.hero.add(ring); this.hero.userData.ring = ring; this.dyn.add(this.hero); } this.hero.position.set(x, 0.05, z); this.heroPath = null; this.hero.userData.walking = false; }
  heroWalk(pts, onDone, startD, meta) { if (!this.hero) this.setHero(pts[0][0], -pts[0][1]); this.heroPath = { pts, len: World.plen(pts), d: startD || 0, onDone, meta: meta || null }; this.hero.userData.walking = true; const p = World.posOn(pts, this.heroPath.d); this.hero.position.set(p.x, 0.05, -p.y); }
  heroCancel() { this.heroPath = null; if (this.hero) this.hero.userData.walking = false; }
  static nearestOnPath(pts, x, y) { let best = { d: 0, dist: 1e9, x: pts[0][0], y: pts[0][1] }; let acc = 0; for (let i = 1; i < pts.length; i++) { const a = pts[i - 1], b = pts[i]; const vx = b[0] - a[0], vy = b[1] - a[1]; const L2 = vx * vx + vy * vy || 1e-9; let t = ((x - a[0]) * vx + (y - a[1]) * vy) / L2; t = Math.max(0, Math.min(1, t)); const px = a[0] + vx * t, py = a[1] + vy * t; const dist = Math.hypot(px - x, py - y); if (dist < best.dist) best = { d: acc + Math.sqrt(L2) * t, dist, x: px, y: py }; acc += Math.sqrt(L2); } return best; }
  static subPath(pts, d0, d1) { // polyline between two distances (d0 > d1 gives reversed order)
    const rev = d0 > d1; const lo = Math.min(d0, d1), hi = Math.max(d0, d1); const out = []; const p0 = World.posOn(pts, lo); out.push([p0.x, p0.y]); let acc = 0; for (let i = 1; i < pts.length; i++) { const a = pts[i - 1], b = pts[i]; const L = Math.hypot(b[0] - a[0], b[1] - a[1]); if (acc + L > lo && acc + L < hi) out.push([b[0], b[1]]); acc += L; } const p1 = World.posOn(pts, hi); out.push([p1.x, p1.y]); return rev ? out.reverse() : out; }
  heroScreen() { if (!this.hero) return null; const v = this.hero.position.clone(); v.y += 3; return this.project(v); }
