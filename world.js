  ...
  _gestures() {
    ...
    el.addEventListener('pointerdown', e => { try { el.setPointerCapture(e.pointerId); } catch (_) {} const r0 = el.getBoundingClientRect(); const hs = this.heroDraggable && this.hero && !this.heroPath ? this.heroScreen() : null;
      if (hs && ptrs.size === 0 && Math.hypot(hs.x - (e.clientX - r0.left), hs.y - (e.clientY - r0.top)) < 34) { // 캐릭터 위 길게 누르면(0.3초) 끌기 시작
        ...
        this._holdT = setTimeout(() => { const cur = ptrs.get(pid); if (ptrs.size === 1 && cur && Math.hypot(cur.x - sx, cur.y - sy) < 8) { ptrs.delete(pid); last = null; this.dragHero = { id: pid, orig: this.hero.position.clone() }; if (this.onHeroHold) this.onHeroHold(); } }, 300); } ptrs.set(e.pointerId, { x: e.clientX, y: e.clientY }); if (ptrs.size === 1) { last = { x: e.clientX, y: e.clientY }; this.dragged = false; } if (ptrs.size === 2) { const [a, b] = [...ptrs.values()]; pd = Math.hypot(a.x - b.x, a.y - b.y); pa = Math.atan2(a.y - b.y, a.x - b.x); twist = 0; rotOn = false; } this._focusId = null; });
    el.addEventListener('pointermove', e => { if (this.dragHero && this.dragHero.id === e.pointerId) { const r0 = el.getBoundingClientRect(); const g = this.groundAt(e.clientX - r0.left, e.clientY - r0.top); if (g) { this.hero.position.set(g.x, 0.05, g.z); } return; } if (!ptrs.has(e.pointerId)) return; ptrs.set(e.pointerId, { x: e.clientX, y: e.clientY }); if (ptrs.size === 1 && last) { const dx = e.clientX - last.x, dy = e.clientY - last.y; if (Math.abs(dx) + Math.abs(dy) > 3) this.dragged = true; this.panPx(dx, dy); last = { x: e.clientX, y: e.clientY }; } else if (ptrs.size === 2) { const [a, b] = [...ptrs.values()]; const d = Math.hypot(a.x - b.x, a.y - b.y), an = Math.atan2(a.y - b.y, a.x - b.x); const r = el.getBoundingClientRect(); const cx = (a.x + b.x) / 2 - r.left, cy = (a.y + b.y) / 2 - r.top;
        ...
        pd = d; pa = an; } });
    const up = e => { clearTimeout(this._holdT); if (this.dragHero && this.dragHero.id === e.pointerId) { const d = this.dragHero; this.dragHero = null; if (this.onHeroDrop) this.onHeroDrop(this.hero.position.clone(), d.orig); return; } ptrs.delete(e.pointerId); if (ptrs.size < 2) pd = 0; if (ptrs.size === 0) last = null; };
